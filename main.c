...
void readWavData(int fileId, sWavHeader *header)
{
	...
	for (uint32_t i = 0; i < numOfSamples; ++i) {
		...
		if ((leftChannel[i] < lowLimit) ||
		    (leftChannel[i] > highLimit)) {
		...
		if (header->NumChannels > 1) {
			...
			// check if value was in range
			if ((rightChannel[i] < lowLimit) ||
			    (rightChannel[i] > highLimit)) {
				printf("**value out of range\n");
			}
		}
	}
...
void writeAsMatlabFile(int16_t *leftChannel, int16_t *rightChannel,
		       sWavHeader *header)
{
	FILE *pFile;
