...
struct HEADER {
	...
	unsigned int
		format_type; // format type. 1-PCM, 3- IEEE float, 6 - 8bit A law, 7 - 8bit mu law
	unsigned int channels; // no.of channels
	unsigned int sample_rate; // sampling rate (blocks per second)
	unsigned int byterate; // SampleRate * NumChannels * BitsPerSample/8
	unsigned int block_align; // NumChannels * BitsPerSample/8
	unsigned int
		bits_per_sample; // bits per sample, 8- 8bits, 16- 16 bits etc
	unsigned char data_chunk_header[4]; // DATA string or FLLR string
	unsigned int
		data_size; // NumSamples * NumChannels * BitsPerSample/8 - size of the next chunk that will be read
};
...
int main(int argc, char **argv)
{
	...
	read = fread(header.fmt_chunk_marker, sizeof(header.fmt_chunk_marker),
		     1, ptr);
	...
	read = fread(header.data_chunk_header, sizeof(header.data_chunk_header),
		     1, ptr);
		...
		if (c == 'Y' || c == 'y') {
			...
			if ((bytes_in_each_channel * header.channels) !=
				...
				printf("Error: %ld x %ud <> %ld\n",
				       bytes_in_each_channel, header.channels,
				       size_of_each_sample);
